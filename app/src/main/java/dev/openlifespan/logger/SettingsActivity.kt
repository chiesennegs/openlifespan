...
class SettingsActivity : Activity() {
    ...
    private fun addData(root: LinearLayout) { root.addView(label("Portable data")); root.addView(action("Export backup") { pendingExport = SessionTransfer.toJson(store.load()); create("application/json", "openlifespan-backup.json") }); root.addView(action("Export CSV") { pendingExport = SessionTransfer.toCsv(store.load()); create("text/csv", "openlifespan-sessions.csv") }); root.addView(action("Import activity") { restoreMode = false; openImport() }); root.addView(action("Restore backup") { restoreMode = true; openImport() }); val all = store.load(); val bytes = getFileStreamPath("openlifespan-sessions.json").length(); val mock = all.count { it.isMock }; val oldest = all.minByOrNull { it.capturedAtMillis }; root.addView(TextView(this).apply { text = buildString { if (mock > 0) append("Mock data: $mock sessions\n"); append("Current data file: ${MockDataGenerator.formatBytes(bytes)}"); if (mock > 0) append("\nAverage mock month: ${MockDataGenerator.formatBytes(bytes / 36)}"); oldest?.let { append("\nOldest record: ${it.displayTitle()} (${if (it.isMock) "Mock" else "Real"})") } }; textSize = 14f; setTextColor(this@SettingsActivity.muted); setPadding(0, dp(16), 0, 0) }) }
    ...
    private fun addSystem(root: LinearLayout) { root.addView(label("System")); root.addView(action("Reset BLE session") { sendBroadcast(Intent(MainActivity.ACTION_RESET_BLE)) }); root.addView(action("Probe console firmware") { sendBroadcast(Intent(MainActivity.ACTION_PROBE_FIRMWARE)); showSheet("Firmware probe", "A read-only firmware request was sent. Keep the console connected, then open Bluetooth / GATT diagnostics to see the response.") }); root.addView(action("Bluetooth / GATT diagnostics") { showSheet("Bluetooth / GATT diagnostics", try { openFileInput("openlifespan-log.txt").bufferedReader().use { it.readText() } } catch (_: Exception) { "No GATT diagnostic entries yet." }) }); root.addView(action("Clear diagnostics log") { showSheet("Clear diagnostics log?", "This removes the active log and its compressed rollover archive.", "Clear") { deleteFile("openlifespan-log.txt"); deleteFile("openlifespan-log-previous.gz") } }); root.addView(action("About & support") { showSheet("OpenLifeSpan", "Version 0.1.0\nCreated by MooseAI, LLC\nmooseaillc.com\nsupport@mooseaillc.com") }); root.addView(action("Licenses") { showSheet("Licenses", "Copyright 2026 MooseAI, LLC\n\nOpenLifeSpan is licensed under Apache License 2.0. This release contains no bundled third-party runtime libraries requiring additional notices.\n\nLifeSpan is a trademark of its respective owner and is used only to identify compatible equipment.") }); root.addView(action("Load Mock Data") { store.replaceAll(store.load().filterNot { it.isMock } + MockDataGenerator.generate()); getSharedPreferences("settings", 0).edit().putBoolean("mockData", true).apply(); showSheet("Mock data loaded", "Three years of randomized treadmill sessions are ready for chart testing.") }); root.addView(action("Unload Mock Data") { val removed = store.load().count { it.isMock }; store.replaceAll(store.load().filterNot { it.isMock }); getSharedPreferences("settings", 0).edit().putBoolean("mockData", false).apply(); showSheet("Mock data unloaded", "Removed $removed mock sessions. Real sessions were preserved.") }) }
    ...
    private fun openImport() { startActivityForResult(Intent(Intent.ACTION_OPEN_DOCUMENT).apply { type = "application/json"; addCategory(Intent.CATEGORY_OPENABLE) }, OPEN) }
    private fun appendImportLog(message: String) { openFileOutput("openlifespan-log.txt", MODE_APPEND).bufferedWriter().use { it.append("import $message\n") } }
    @Deprecated("Compatibility with minSdk 26") override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) { super.onActivityResult(requestCode, resultCode, data); if (resultCode != RESULT_OK) return; val uri = data?.data ?: return; if (requestCode == CREATE) contentResolver.openOutputStream(uri)?.bufferedWriter()?.use { it.write(pendingExport) }; if (requestCode == OPEN) try { val imported = contentResolver.openInputStream(uri)?.use { SessionTransfer.read(it) }.orEmpty(); val result = if (restoreMode) SessionTransfer.ImportResult(imported, imported.size, store.load().size, 0) else SessionTransfer.mergeImport(store.load(), imported); store.replaceAll(result.sessions); val message = if (restoreMode) "Restored ${result.added} sessions; replaced ${result.replaced} local sessions." else "Imported ${result.added}; replaced ${result.replaced}; skipped ${result.skipped}."; appendImportLog("complete $message"); showSheet(if (restoreMode) "Restore complete" else "Import complete", message) } catch (e: Exception) { appendImportLog("failed ${e.message}"); showSheet("Import failed", "Validation stopped the import. See Bluetooth / GATT diagnostics for the record and line details.") } }
}
